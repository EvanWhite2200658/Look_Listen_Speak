...
class AvatarScreen:
    ...
    def __init__(self, width: int = 500, height: int = 300) -> None:
        ...
        self._canvas.pack()
        self._status_text = self._canvas.create_text(width // 2, height - 30, text="idle", font=("Arial", 16))
    ...
    def start(self) -> None:
        self._draw_face()
        self._tick()
        thread = threading.Thread(target=self._root.mainloop, daemon=True, name="AvatarScreen")
        thread.start()
